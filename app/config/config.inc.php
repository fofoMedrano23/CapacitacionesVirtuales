<?php
if (!defined("PJ_HOST")) define("PJ_HOST", "localhost");
if (!defined("PJ_USER")) define("PJ_USER", "class-system");
if (!defined("PJ_PASS")) define("PJ_PASS", "Rmc23831983");
if (!defined("PJ_DB")) define("PJ_DB", "class-system");
if (!defined("PJ_PREFIX")) define("PJ_PREFIX", "");
if (!defined("PJ_INSTALL_FOLDER")) define("PJ_INSTALL_FOLDER", "/Class-system/");
if (!defined("PJ_INSTALL_PATH")) define("PJ_INSTALL_PATH", "/opt/lampp/htdocs/Class-system/");
if (!defined("PJ_INSTALL_URL")) define("PJ_INSTALL_URL", "http://localhost/Class-system/");
if (!defined("PJ_SALT")) define("PJ_SALT", "20GYEAQ7");
if (!defined("PJ_INSTALLATION")) define("PJ_INSTALLATION", "");
?>